...
/**
 * Base options every yt-dlp invocation should include. Merges in cookies
 * and, if configured, a proxy (YTDLP_PROXY - e.g. a residential proxy
 * service's URL, "http://user:pass@host:port"). Routing through a
 * residential IP is the one real lever against YouTube's datacenter-IP
 * bot detection when cookies alone aren't enough - see friendlyError()'s
 * bot-check message. No proxy configured = no change in behavior.
 */
