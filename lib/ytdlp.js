...
export async function resolveCookies(req) {
  ...
  if (encoded && encoded.trim()) {
    ...
    try {
      ...
      console.warn("[yt-dlp] failed to decode x-ytdlp-cookies header:", e.message);
    }
    if (visitorCookies && visitorCookies.trim()) {
      const trimmed = toNetscapeCookieFile(visitorCookies).slice(0, MAX_COOKIES_BYTES);
      const filePath = path.join(os.tmpdir(), `ytlabs-cookies-${randomUUID()}.txt`);
      try {
...
export function friendlyError(err) {
  const raw = (err?.stderr || err?.message || String(err) || "Unknown error")
    .split("\n")
